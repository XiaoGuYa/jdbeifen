...
const JX_CODE = $.isNode() ? (process.env.JX_CODE ? process.env.JX_CODE : '3tU8v1a') : '3tU8v1a';
let UA;
if ($.isNode()) {
...
function getCoupons(shareId = '',type = 1) {
    return new Promise(resolve => {
        let options = {
            url: `https://api.m.jd.com/api?functionId=getCoupons&appid=u&_=${Date.now()}&loginType=2&body=${encodeURIComponent(JSON.stringify({
                "platform": 4,
                "unionActId": "31134",
                "actId": $.actId,
                "d": `${$.jxCode}`,
                ...
                "type": type,
                "eid": $.eid
            }))}&client=apple&clientVersion=8.3.6&h5st=undefined`,
            headers: {
                "Host": "api.m.jd.com",
...
function shareUnionCoupon() {
    return new Promise(resolve => {
        let options = {
            url: `https://api.m.jd.com/api?functionId=shareUnionCoupon&appid=u&_=${Date.now()}&loginType=2&body=${encodeURIComponent(JSON.stringify({
                "unionActId": "31134",
                "actId": $.actId,
                "platform": 4,
                ...
                "supportLuckyCode": 0,
                "eid": $.eid
            }))}&client=apple&clientVersion=8.3.6`,
            headers: {
                "Host": "api.m.jd.com",
                "Origin": "https://prodev.m.jd.com",
                "User-Agent": UA,
...
function getUrl() {
    return new Promise(resolve => {
        const options = {
            url: `https://u.jd.com/${$.jxCode}?s=${$.shareCode}`,
            followRedirect: false,
            headers: {
                "Host": "u.jd.com",
                "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
